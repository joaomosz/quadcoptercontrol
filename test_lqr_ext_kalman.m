% Tests the Craziflie model using LQR controller and extended kalman filter
% estimate for partially and/or noisy observations
% THIS MUST HAVE  A LITTLE BIT OF NOISE POWER

%Fully observed performs well (similar to lqr). Noisy still is able to
%control.
%Half observed still performs well. Only "fails" for high noise...
...
dt = 0.01; %step size
t_end = 20;
t = 0:dt:t_end; %simulation times array
...
P = 1*eye(12); %from process X itself
...
for i=1:length(t)
    ...
    %estimating full and noiseless state with extended kalman filter
    [X_filtered, P] = extendedKalmanFilter(P, X_filtered, u, dt, Wn, Wd, X_observed, observation);
